...
exports.contentCalendarBot = onSchedule(
  ...
  async () => {
    ...
    const clients = await getActiveClients('content_calendar');
    logger.info(`contentCalendarBot: running for ${clients.length} clients`);

    for (const client of clients) {
      try {
        const ideas = await generateWithGemini(
          `You are a social media strategist. Generate 8 content ideas for next week for this business:\nBusiness: ${client.businessName || client.name}\nIndustry: ${client.industry || 'general'}\nTarget audience: ${client.targetAudience || 'general audience'}\nTone: ${client.toneOfVoice || 'friendly'}\nDescription: ${client.businessDescription || ''}\n\nFormat as a numbered list. Each idea: one line with the platform emoji, content type, and topic.\nExample: 📸 Instagram Post — Behind-the-scenes of our process\nNo preamble. South African context where relevant.`
        );

        await sendEmailViaResend({
          to: client.email,
          subject: `Your Weekly Content Ideas — Drift Studio`,
          html: `<h2>This week's content ideas for ${client.businessName || client.name}</h2><pre style="font-family:sans-serif;line-height:1.8">${ideas}</pre><p style="color:#888;font-size:12px">Generated by Drift Studio's Content Bot. Reply to this email with feedback.</p>`,
        });

        await db.collection('clientContent').doc(client.id).collection('weeklyIdeas').add({
          ideas,
          generatedAt: admin.firestore.FieldValue.serverTimestamp(),
          week: new Date().toISOString().slice(0, 10),
        });

        logger.info(`contentCalendarBot: sent to ${client.email}`);
      } catch (err) {
        logger.error(`contentCalendarBot error for client ${client.id}:`, err.message);
      }
    }
  }
);
