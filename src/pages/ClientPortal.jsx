...
import { Lock, User, BarChart3, FileText, TrendingUp, Mail, Eye, LogOut, Plus, Calendar, Clock, ExternalLink, FolderOpen } from 'lucide-react';
import { signIn, logOut, onAuthChange, resetPassword } from '../firebase/auth';
import { getClient, subscribeToCampaigns, getClientReports, subscribeToContentCalendars, subscribeToScheduledPosts } from '../firebase/firestore';
  ...
  useEffect(() => {
    const unsubscribe = onAuthChange((firebaseUser) => {
      if (firebaseUser) {
        ...
        // Subscribe to content calendars
        const unsubCalendars = subscribeToContentCalendars(firebaseUser.uid, (data) => {
          setContentCalendars(data);
        });

        // Subscribe to scheduled posts
        const unsubPosts = subscribeToScheduledPosts(firebaseUser.uid, (data) => {
          setScheduledPosts(data);
        });

        return () => { unsubCampaigns(); unsubCalendars(); unsubPosts(); };
      } else {
        setUser(null);
        setClientData(null);
        setCampaigns([]);
